    ...
    return `
      <div class="booking-card" style="margin-bottom:16px">
        ...
        <div class="ticket-grid" style="margin-top:14px">
          <div class="ticket-item"><span>Booking ID</span><strong>${escHtml(booking.bookingId || "-")}</strong></div>
          <div class="ticket-item"><span>Route</span><strong>${escHtml(booking.from || "-")} → ${escHtml(booking.to || "-")}</strong></div>
          <div class="ticket-item"><span>Date</span><strong>${escHtml(booking.date || "-")}</strong></div>
          <div class="ticket-item"><span>Seats</span><strong>${escHtml((booking.seats || []).join(", "))}</strong></div>
          <div class="ticket-item"><span>Passenger</span><strong>${escHtml(booking.passenger || "-")}</strong></div>
          <div class="ticket-item"><span>Payment</span><strong>${escHtml(booking.paymentMethod || "-")}</strong></div>
        </div>

        ${booking.status === "CONFIRMED"
          ? `<div class="actions">
               <button class="btn btn-dark cancel-btn" data-id="${booking._id}">Cancel Booking</button>
             </div>`
  ...
  document.querySelectorAll(".cancel-btn").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        alert("Server error while cancelling. Please try again.");
        btn.textContent = "Cancel Booking";
        btn.disabled = false;
      }
    });
...
async function loadBookings() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/bookings?phone=${encodeURIComponent(phone)}`,
